...
class Sensor:
    def __init__(self,ip =TCP_IP, names=[]):
        ...
        self.names=names
        self.channelsstart=[ 0x9C40, 0x9C41, 0x9C42, 0x9C43, 0x9C44, 0x9C45, 0x9C46, 0x9C47]
    ...
    def getName(self,channel):
        if channel< len(self.names):
            return self.ip+"/"+str(channel)+"."+self.names[channel]
        else:
            return self.ip+"/"+str(channel)
    ...
    def getInt(self,adress):
        l=1
        req = struct.pack('>LHccHH', 0, 6,chr(1).encode('ascii'),chr(3).encode('ascii'), adress, l)
